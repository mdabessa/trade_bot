...
class ManagerBacktest(Manager):
    def __init__(self, logger: Logger, coins_symbols: list,
                usdt_quantity: float = 100.0, fee: float = 0.1, 
                keep_historic: int = 1440) -> None:
...
class ManagerHistoricalBacktest(ManagerBacktest):
    def __init__(self, logger: Logger, coins_symbols: list,
                database_historic: str, start_date: str,
                end_date: str, summary_fname: str,  
                usdt_quantity: float = 100, fee: float = 0.1, 
                keep_historic: int = 1440) -> None:
        
        super().__init__(logger, coins_symbols, usdt_quantity, fee, keep_historic)

        self.summary = {'daily_close':[], 'daily_close_btc':[], 'orders_historic':[]}
    ...
    def att_price(self) -> None:
        ...
        if self.date >= self.end_date:
            ...
            self.logger(f'Total simulation time: {nround(self.simulation_time.total())} secs')
            return False
