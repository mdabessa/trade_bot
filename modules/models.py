...
class Coin(Model):
    ...
    def __init__(self, symbol: str, price: float, historic: list = None, 
                keep_historic: int = 50, lot_size: str = '0.001') -> None:
        super().__init__(symbol=str(symbol), price=float(price), historic=historic, 
                        keep_historic=int(keep_historic), lot_size=str(lot_size))
...
class Trade(Model):
    if session:
        ...
        coin_symbol =  Column(String, ForeignKey('coins.symbol'), primary_key=True)
    ...
    def __init__(self, coin: Coin, price: float, age: int, id: int) -> None:
        super().__init__(coin=coin, coin_symbol=str(coin.symbol), price=float(price), age=int(age), id=int(id))
...
class Balance(Model):
    ...
    def __init__(self, coin: Coin, quantity: float = 0.0, fee: float = 0.0) -> None:
        super().__init__(coin=coin, coin_symbol=coin.symbol, quantity=float(quantity), fee=float(fee))
