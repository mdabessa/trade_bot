...
def diff_daily(data, plot=False):
    ...
    print(f'start: {usdt[0]} USDT, finish: {usdt[-1]} USDT, max: {max(usdt)} USDT, min: {min(usdt)} USDT')
    print(f'start: {btc[0]} BTC, finish: {btc[-1]} BTC, max: {max(btc)} BTC, min: {min(btc)} BTC')
...
def best_and_worst(data):
    ...
    print(f'Best: {buy[best]["symbol"]} buy: {buy[best]["price"]} and sell: {sell[best]["price"]} diff: +{(diffs[best]*100)-100}%')
    print(f'Worst: {buy[worst]["symbol"]} buy: {buy[worst]["price"]} and sell: {sell[worst]["price"]} diff: -{((1/diffs[worst])*100)-100}%')
...
def positives_and_negatives(data):
    ...
    p = sum(c)
    print(f'Positives orders: {p}, Negatives orders: {len(c)-p}, Total orders: {len(c)}, Diff: {(p/len(c))*100}%')
...
def coin_quality(data):
    ...
    coins = {}
    for b in buy:
        s = list(filter(lambda x: x['type'] == 'sell' and x['id'] == b['id'], orders))[0]
    ...
    for r in res:
        ...
        print(f'{key}{" "*(5-len(key))} || Positives: {pos}, Total: {total} = {(pos/total)*100}%')
